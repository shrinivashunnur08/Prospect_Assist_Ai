...
def generate_customer(persona_key, months=9, seed=None, name=None):
    ...
    # --- declared income + expressed-interest events (SEPARATE rng stream so the
    # transaction/label sequence above stays byte-identical to registered metrics) ---
    rng2 = random.Random(((seed or 0) + 1) * 100003)
    base_income = salary_amt if salary_amt else (
        sum(t["amount"] for t in txns if t["type"] == "CREDIT") / max(months, 1))
    declared_factor = {
        "salaried_regular": (0.95, 1.20),   # mild rounding-up on forms
        "over_leveraged":  (1.05, 1.35),    # stretches the truth to qualify
        "gig_worker":      (1.00, 1.50),    # optimistic annualization
        "self_employed":   (0.50, 0.90),    # understates (tax-shy) — still creditworthy
        "dormant":         (0.95, 1.10),
        "income_padder":   (3.00, 4.00),    # declares the padded fiction as fact
    }[persona_key]
    declared = base_income * rng2.uniform(*declared_factor)
    engage_prob = {"salaried_regular": 0.30, "self_employed": 0.25, "gig_worker": 0.25,
                   "over_leveraged": 0.35, "dormant": 0.05, "income_padder": 0.40}[persona_key]
    engagement = None
    if rng2.random() < engage_prob:   # "genuinely interested" signal (Tier-2 moment)
        engagement = {
            "event": rng2.choice(["used the bank's EMI calculator",
                                  "clicked a pre-approved offer SMS",
                                  "asked about loans at the branch"]),
            "days_ago": rng2.randint(1, 14),
        }
    # contact details, carried where ReBIT holder records actually carry them
    mobile = "+91 9" + "".join(str(rng2.randint(0, 9)) for _ in range(9))
    email = name.lower().replace(" ", ".").replace("..", ".") + rng2.choice(
        ["@gmail.com", "@yahoo.in", "@outlook.com"])
    # ReBIT FI-Deposit-shaped envelope
    return {
        "customerName": name,
        "persona": persona_key,
        "declaredIncomeMonthly": round(declared, 2),
        ...
        "account": {
            ...
            "type": "deposit",
            "fiType": "DEPOSIT",
            "profile": {"holders": {"holder": [{"name": name, "mobile": mobile,
                                                "email": email, "ckycCompliance": True}]}},
