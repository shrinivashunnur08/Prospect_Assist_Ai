...
GLOSSARY = {
    ...
    "Hot lead 🔥": "The customer EXPRESSED interest recently — used the EMI calculator, clicked a "
                   "pre-approved offer, or asked at the branch. The PS's 'genuinely interested' signal: "
                   "behavioral intent PLUS a hand raised.",
    "Declared vs assessed": "What the customer CLAIMS to earn vs what their cash-flows prove. "
                            "Overstated ⇒ underwrite on the assessed figure; understated (common for "
                            "self-employed) ⇒ hidden creditworthiness the bank would otherwise miss.",
}
...
st.session_state.setdefault("tbl_nonce", 0)
# row-click navigation: Ranked Leads sets _goto, consumed here BEFORE widgets exist
if "_goto" in st.session_state:
    g = st.session_state.pop("_goto")
    st.session_state["nav"] = "🔍 Customer Deep-Dive"
    st.session_state["dd_mode"] = "📇 Let me choose"
    st.session_state["dd_pick"] = g
    st.session_state["tbl_nonce"] += 1  # fresh table key => selection cleared
...
def score_all(prod):
    ...
    for i, c in enumerate(cohort[:80]):  # demo subset
        ...
        rows.append(dict(idx=i, Name=s["name"], Phone=s["contact"]["mobile"],
                         Segment=s["persona"].replace("_", " ").title(),
...
if page == "📋 Ranked Leads":
    ...
    c3.metric("Hot leads 🔥", int(((leads.Status == "✅") & (leads.Hot == "🔥")).sum()),
              help=GLOSSARY["Hot lead 🔥"])
    c4.metric("Capacity-gated 🚫", int((leads.Status == "🚫").sum()),
              help=f"FOIR ≥ {FOIR_CEILING:.0%} or income < ₹{MIN_INCOME:,} — cannot safely service a new EMI")
    c5.metric("Trust-flagged 🛡️", int((leads.Status == "🛡️").sum()),
    ...
    st.markdown("👆 **Click any row** to open that customer's full Deep-Dive (verdict, offer, contact, call logging).")
    ev = st.dataframe(
        show[["Rank", "Name", "Phone", "Segment", "IntentScore", "AssessedIncome", "Confidence",
              "FOIR", "Trust", "PreApproved", "Status", "Hot", "Contacted", "TopReason"]],
        use_container_width=True, hide_index=True, height=430,
        on_select="rerun", selection_mode="single-row",
        key=f"leads_tbl_{product}_{st.session_state.tbl_nonce}",
        column_config={
            "Phone": st.column_config.TextColumn("📱 Phone", help="From the CKYC/account-holder record (synthetic in demo)"),
            ...
            "Hot": st.column_config.TextColumn("🔥", help=GLOSSARY["Hot lead 🔥"]),
    ...
    if ev.selection.rows:
        st.session_state["_goto"] = show.iloc[ev.selection.rows[0]]["Name"]
        st.rerun()
    st.markdown("**⚡ Quick open — top call-ready leads:**")
    qcols = st.columns(8)
    for i, (_, r) in enumerate(elig_ranked.head(8).iterrows()):
        if qcols[i].button(f"#{int(r['Rank'])} · {r['Name'].split()[0]}",
                           key=f"open_{product}_{r['Name']}", help=f"Open {r['Name']}'s Deep-Dive",
                           use_container_width=True):
            st.session_state["_goto"] = r["Name"]
            st.rerun()
    st.info(
...
def render_customer(s, rank=None, total=None, key_prefix="dd"):
    ...
    st.markdown(f"#### 📱 {s['contact']['mobile']} &nbsp;·&nbsp; ✉️ {s['contact']['email']}")
    ...
    with a:
        ...
        ic = s.get("income_check")
        if ic:
            if ic["verdict"] == "OVERSTATED":
                st.error(f"🧾 **Declared vs assessed:** customer declares ₹{ic['declared']:,.0f}/month — "
                         f"cash-flows support only ₹{s['income']['monthly_income']:,.0f} "
                         f"(**{ic['gap']:.0%} overstated**). Underwrite on the assessed figure.")
            elif ic["verdict"] == "UNDERSTATED":
                st.info(f"🧾 **Declared vs assessed:** customer declares ₹{ic['declared']:,.0f}/month but "
                        f"cash-flows prove ₹{s['income']['monthly_income']:,.0f} "
                        f"(**{-ic['gap']:.0%} understated** — common for the self-employed). "
                        "Hidden creditworthiness a form-based process would miss.")
            else:
                st.caption(f"🧾 Declared income ₹{ic['declared']:,.0f}/month is consistent with assessed cash-flows ✓")
...
if page == "🔍 Customer Deep-Dive":
    ...
    pick_mode = st.radio(
        "quick pick",
        ["📇 Let me choose", "☎️ Worklist — next lead to call", "🔥 A hot lead (expressed interest)",
         "🕵️ A fraud catch", "⛔ An over-leveraged reject"],
        index=0, horizontal=True, label_visibility="collapsed", key="dd_mode",
    )
    names = leads["Name"].tolist()
    worklist = elig_ranked[~elig_ranked["Name"].isin(contacted_names)]
    if pick_mode == "📇 Let me choose":
        if st.session_state.get("dd_pick") not in names:
            st.session_state.pop("dd_pick", None)
        pick = st.selectbox("Choose a customer", names, key="dd_pick")
    ...
    elif pick_mode == "🔥 A hot lead (expressed interest)" and ((leads.Status == "✅") & (leads.Hot == "🔥")).any():
        pick = leads[(leads.Status == "✅") & (leads.Hot == "🔥")].iloc[0]["Name"]
...
if page == "💰 Business Impact":
    ...
    # the PS's opening claim, tested live: "relies on TRADITIONAL METRICS → low conversions"
    trad_mask = (leads["Segment"] == "Salaried Regular") & (leads["AssessedIncome"] > 50000)
    trad_rate = float(np.mean([cohort[int(i)]["label_" + product] for i in leads[trad_mask]["idx"]])) if trad_mask.any() else 0.0
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Spray-and-pray (call everyone)", f"{base_rate:.0%}")
    c2.metric("Traditional filter (salaried, ₹50k+)", f"{trad_rate:.0%}",
              help="How banks target today: employment type + income band. "
                   "The PS's 'traditional metrics' — measured on this same cohort.")
    c3.metric(f"Prospect Assist top-{top_n} list", f"{top_rate:.0%}",
              f"{(top_rate / max(base_rate, 1e-9)):.1f}× vs spray-and-pray")
    c4.metric("Calls saved", f"{(1 - top_n / len(leads)):.0%}",
              "of the base never needs a cold call")
    comp = go.Figure(go.Bar(
        x=["Spray-and-pray<br>(call everyone)", "Traditional metrics<br>(salaried + income band)",
           f"Prospect Assist<br>(top-{top_n} behavioral list)"],
        y=[base_rate * 100, trad_rate * 100, top_rate * 100],
        marker_color=["#9E9E9E", "#F9A825", "#1B5E20"],
        text=[f"{base_rate:.0%}", f"{trad_rate:.0%}", f"{top_rate:.0%}"], textposition="outside"))
    comp.update_layout(title=f"Conversion by targeting method — {PRODUCT_LABEL[product]}, same cohort, same RM effort",
                       yaxis_title="conversion %", showlegend=False, height=380,
                       yaxis_range=[0, max(top_rate * 100 * 1.25, 10)])
    st.plotly_chart(comp, use_container_width=True)
    st.caption(
        "This is the problem statement's own opening line, measured: targeting on traditional metrics "
        "(employment + income band) barely beats random, because it sees neither intent nor true capacity. "
        "Behavioral targeting sees both."
    )
    st.markdown(
        f"""
